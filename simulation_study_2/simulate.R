#' Simulate data for simulation study 2.
#'
...
#'
simulate_data <- function(seed = 5462, N = 1e3, m = 5, P = 5, outcome_type = "continuous") {
  set.seed(seed)
  ...
  g <- 1 + 
    10 * matrix(W[, 1], ncol = m, nrow = N, byrow = FALSE) * matrix(hospital_effect1, ncol = m, nrow = N, byrow = TRUE) - 
    0.2 * matrix(W[, 2], ncol = m, nrow = N, byrow = FALSE) - 
  ...
  Qbar <- 0 + 
    2 * matrix(W[, 1], ncol = m, nrow = N, byrow = FALSE) -
    matrix((W[, 2] - 0.5)^2, ncol = m, nrow = N, byrow = FALSE) +
    matrix(ifelse(W[, 3] > 0.5, 1, 0), ncol = m, nrow = N, byrow = FALSE) +
    matrix(W[, 5], ncol = m, nrow = N, byrow = FALSE) - 
    2 * matrix(hospital_effect1, ncol = m, nrow = N, byrow = TRUE)
